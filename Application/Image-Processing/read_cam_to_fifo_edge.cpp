  ...
  while (1) {
    ...
    for (int i = 0; i < frame.rows; i++) {
      for (int j = 0; j < frame.cols; j++) {
        buf[number_of_bytes++] = frame.at<Vec3b>(i, j)[0];
        buf[number_of_bytes++] = frame.at<Vec3b>(i, j)[1];
        buf[number_of_bytes++] = frame.at<Vec3b>(i, j)[2];
        if (i == 0 && j == 0) {
            buf[number_of_bytes++] = (char) 1;
            printf("write (%d) start\n", (int) buf[number_of_bytes-1]);
        }
        else if (i == (frame.rows - 2) && j == 0) {
            buf[number_of_bytes++] = (char) 2;
            printf("write (%d) pre\n", (int) buf[number_of_bytes-1]);
        }
        else if (i == (frame.rows - 1) && j == 0) {
