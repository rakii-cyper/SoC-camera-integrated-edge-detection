...
int main(int argc, char *argv[]) {
  ...
  int count_frame = 0;
  VideoWriter video("outcpp.avi", cv::VideoWriter::fourcc('M','J','P','G'), 30, Size(frame_width, frame_height), false);
  ...
  while (count_frame <= 50) {
    int cols = 0;
    int rows = 0;
    int len = 0;
    Mat frame = Mat::zeros(Size(frame_width, frame_height), CV_8UC1);
    while (cols < frame_width && rows < frame_height) {
      ...
      while (len < rc) {
          frame.at<uchar>(rows, cols) = (int) buf[len+1];
          cols++;
          len = len + 4;
          if (cols == 640) {
            cols = 0;
